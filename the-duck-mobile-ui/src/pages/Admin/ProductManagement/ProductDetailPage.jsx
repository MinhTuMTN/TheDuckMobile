...
function ProductDetailPage() {
  ...
  const emptyProductVersionRows =
    productVersionPage > 0
      ? Math.max(
          0,
          (1 + productVersionPage) * productVersionRowsPerPage -
            productVersionRows.length
        )
      : 0;

  const emptyRateRows =
    ratePage > 0
      ? Math.max(0, (1 + ratePage) * rateRowsPerPage - rateRows.length)
      : 0;
        ...
        <Table sx={{ maxWidth: 1200 }}>
          ...
          <TableBody>
            {(productVersionRowsPerPage > 0
              ? productVersionRows.slice(
                  productVersionPage * productVersionRowsPerPage,
                  productVersionPage * productVersionRowsPerPage +
                    productVersionRowsPerPage
                )
              : productVersionRows
            ).map((row) => (
      ...
      >
        <Table sx={{ maxWidth: 1200 }}>
          ...
          <TableBody>
            {(rateRowsPerPage > 0
              ? rateRows.slice(
                  ratePage * rateRowsPerPage,
                  ratePage * rateRowsPerPage + rateRowsPerPage
                )
              : rateRows
            ).map((row) => (
              <TableRow key={row.name}>
